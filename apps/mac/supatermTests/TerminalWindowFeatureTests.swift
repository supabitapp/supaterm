...
struct TerminalWindowFeatureTests {
  ...
  @Test
  func closeTabsRequestedAsksHostToResolveBatchClose() async {
    let recorder = TerminalCommandRecorder()
    ...
    await store.send(.closeTabsRequested([firstTabID, secondTabID]))

    #expect(recorder.commands == [.requestCloseTabs([firstTabID, secondTabID])])
  }

  @Test
  func tabSelectedCollapsesSidebarSelectionAndSendsSelectCommand() async {
    let recorder = TerminalCommandRecorder()
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    initialState.sidebarTabSelection = .init(
    ...
    await store.send(.tabSelected(secondTabID)) {
      $0.pendingSidebarSelectionActiveTabID = secondTabID
      $0.sidebarTabSelection = .init(
        anchorTabID: secondTabID,
        tabIDs: [secondTabID]
      )
    }

    #expect(recorder.commands == [.selectTab(secondTabID)])
  }

  @Test
  func sidebarTabCommandClickedAddsTabAndSendsSelectCommand() async {
    let recorder = TerminalCommandRecorder()
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    initialState.lastSyncedActiveSidebarTabID = firstTabID
    initialState.sidebarTabSelection = .init(
      anchorTabID: firstTabID,
      tabIDs: [firstTabID]
    )
    ...
    await store.send(.sidebarTabCommandClicked(tabID: secondTabID, activeTabID: firstTabID)) {
      $0.pendingSidebarSelectionActiveTabID = secondTabID
      $0.sidebarTabSelection.anchorTabID = secondTabID
      $0.sidebarTabSelection.tabIDs = [firstTabID, secondTabID]
    }
  ...
  @Test
  func sidebarTabCommandClickedRemovesNonActiveSelectedTab() async {
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    initialState.lastSyncedActiveSidebarTabID = firstTabID
    ...
    await store.send(.sidebarTabCommandClicked(tabID: secondTabID, activeTabID: firstTabID)) {
      $0.sidebarTabSelection.anchorTabID = firstTabID
      $0.sidebarTabSelection.tabIDs = [firstTabID]
    }
  }

  @Test
  func sidebarTabRangeSelectedUsesVisibleOrderAndSendsSelectCommand() async {
    let recorder = TerminalCommandRecorder()
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    let thirdTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    initialState.sidebarTabSelection = .init(
      anchorTabID: firstTabID,
      tabIDs: [firstTabID]
    ...
    await store.send(
      ...
      $0.pendingSidebarSelectionActiveTabID = thirdTabID
      $0.sidebarTabSelection.tabIDs = [firstTabID, secondTabID, thirdTabID]
    }

    #expect(recorder.commands == [.selectTab(thirdTabID)])
  }

  @Test
  func sidebarTabContextMenuRequestedCollapsesSelectionForUnselectedTab() async {
    let recorder = TerminalCommandRecorder()
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    ...
    await store.send(.sidebarTabContextMenuRequested(tabID: secondTabID, activeTabID: firstTabID)) {
      $0.pendingSidebarSelectionActiveTabID = secondTabID
      $0.sidebarTabSelection = .init(
        anchorTabID: secondTabID,
        tabIDs: [secondTabID]
  ...
  @Test
  func sidebarTabDragStartedCollapsesSelectionAndSendsSelectCommand() async {
    let recorder = TerminalCommandRecorder()
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    ...
    await store.send(.sidebarTabDragStarted(secondTabID)) {
      $0.pendingSidebarSelectionActiveTabID = secondTabID
      $0.sidebarTabSelection = .init(
        anchorTabID: secondTabID,
        tabIDs: [secondTabID]
  ...
  @Test
  func sidebarTabSelectionSyncedPreservesPendingMultiSelection() async {
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    initialState.lastSyncedActiveSidebarTabID = firstTabID
    initialState.pendingSidebarSelectionActiveTabID = secondTabID
    initialState.sidebarTabSelection = .init(
      anchorTabID: firstTabID,
      tabIDs: [firstTabID, secondTabID]
    )
    ...
    await store.send(
      .sidebarTabSelectionSynced(
        ...
        visibleTabIDs: [firstTabID, secondTabID]
      )
    ) {
      $0.lastSyncedActiveSidebarTabID = secondTabID
      $0.pendingSidebarSelectionActiveTabID = nil
    }
  }

  @Test
  func sidebarTabSelectionSyncedCollapsesSelectionWhenActiveTabChangesExternally() async {
    let firstTabID = TerminalTabID()
    let secondTabID = TerminalTabID()
    let thirdTabID = TerminalTabID()
    var initialState = TerminalWindowFeature.State()
    ...
    await store.send(
      .sidebarTabSelectionSynced(
        activeTabID: thirdTabID,
        visibleTabIDs: [firstTabID, secondTabID, thirdTabID]
      )
    ) {
      $0.lastSyncedActiveSidebarTabID = thirdTabID
      $0.sidebarTabSelection = .init(
        anchorTabID: thirdTabID,
        tabIDs: [thirdTabID]
      )
    }
  }
