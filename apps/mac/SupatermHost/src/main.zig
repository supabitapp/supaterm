...
fn help(io: std.Io) !void {
    const help_text =
        \\supaterm-host - session persistence for terminal processes
        \\
        ...
        \\
        \\  Examples:
        \\    supaterm-host attach dev
        \\    supaterm-host attach dev vim
        \\
        \\History:
        ...
        \\
        \\  Examples:
        \\    supaterm-host history <session> | tail -100
        \\
        \\Run:
        ...
        \\  (pagers, editors, prompts) from blocking. Use `supaterm-host send` for
        \\  commands that need user input, or pipe data directly:
        \\    echo "data" | supaterm-host run dev cat
        \\
        \\  `-d` will detach from the calling terminal. Use `wait` to track
        \\  its status.
        \\
        \\  Examples:
        \\    supaterm-host run dev ls
        \\    supaterm-host run dev zig build
        \\    supaterm-host run dev grep -r TODO src
        \\    supaterm-host run dev git log --oneline          # pager won't block
        \\    echo "hello" | supaterm-host run dev cat         # piped stdin still works
        \\
        \\    # heredoc
        \\    printf "cat << 'EOF'\r\nHello $USER\r\nToday is $(date).\r\nEOF" | supaterm-host run dev
        \\
        \\    # non-blocking
        \\    supaterm-host run dev -d sleep 10
        \\    supaterm-host wait dev
        \\
        \\Send:
        ...
        \\    printf 'ls -la\r' | supaterm-host send dev
        \\
        \\  Examples:
        \\    printf 'echo hello\r' | supaterm-host send dev
        \\    supaterm-host send dev $(printf '\x03')
        \\    supaterm-host send dev /compact
        \\
        \\Print:
        ...
        \\
        \\  Examples:
        \\    printf '\\r\\nhello\\r\\n' | supaterm-host print dev
        \\    supaterm-host print dev "$(printf '\\r\\nalert\\r\\n')"
        \\
        \\Write:
        ...
        \\
        \\  Examples:
        \\    echo "hello" | supaterm-host write dev /tmp/hello.txt
        \\    cat main.zig | supaterm-host write dev src/main.zig
        \\
        \\Wait:
        ...
        \\
        \\  Examples:
        \\    supaterm-host run -d dev sleep 10
        \\    supaterm-host wait dev
        \\    supaterm-host wait dev other
        \\
        \\Labels:
        ...
        \\
        \\  Examples:
        \\    supaterm-host set dev project=supaterm-host env=dev
        \\    supaterm-host set dev project=            # unset a label
        \\    supaterm-host set . status=fail           # "." resolves to current session
        \\    supaterm-host get dev
        \\    supaterm-host get dev project
        \\    supaterm-host set next "$(supaterm-host get prev)"  # set labels from other session
        \\    supaterm-host list | grep project=supaterm-host
        \\    supaterm-host clear dev
        \\
        \\Environment variables:
        \\  SHELL                Default shell for new sessions
        \\  SUPATERM_HOST_DIR              Socket directory (priority 1)
        \\  XDG_RUNTIME_DIR      Socket directory (priority 2)
        \\  TMPDIR               Socket directory (priority 3)
        \\  SUPATERM_HOST_SESSION          Session name (injected automatically)
        \\  SUPATERM_HOST_SESSION_PREFIX   Prefix added to all session names
        \\  SUPATERM_HOST_DIR_MODE         Sets mode for socket and log directories (octal, defaults to 0700)
        \\  SUPATERM_HOST_LOG_MODE         Sets mode for log files (octal, defaults to 0600)
        \\  SUPATERM_HOST_NO_DETACH_KEY    Disables the ctrl+\ detach shortcut (set to any value)
        \\
    ;
...
fn wait(alloc: std.mem.Allocator, io: std.Io, cfg: *Cfg, matchers: std.ArrayList(socket.SessionMatch)) !void {
    ...
    while (true) {
        ...
        if (max_seen == 0) {
            // `supaterm-host run foo && supaterm-host wait foo` is essentially sequential, so
    ...
    for (sessions.items) |session| {
        ...
        if (session.task_exit_code.? > 0) {
            ...
            try stdout.print("\nSee the logs:\nsupaterm-host history {s}\nsupaterm-host attach {s}\n", .{ session.name, session.name });
            try stdout.flush();
        }
    }
...
fn detachAll(alloc: std.mem.Allocator, io: std.Io, cfg: *Cfg) !void {
    ...
    if (session_name.len == 0) {
        std.log.err("SUPATERM_HOST_SESSION env var not found: are you inside a supaterm-host session?", .{});
        return;
    }
...
fn kill(alloc: std.mem.Allocator, io: std.Io, cfg: *Cfg, session_name: []const u8, force: bool) !void {
    ...
    // and a subsequent `supaterm-host run <name>` can't land in the dying daemon's
    // accept backlog.
...
fn switchSesh(gpa: std.mem.Allocator, io: std.Io, daemon: *Daemon, current_sesh: []const u8) !void {
    // we want daemon.session_name because that's the session name the user provided during supaterm-host attach
...
test "existing-only connection never creates a session" {
    ...
    const session_name = try std.fmt.allocPrint(alloc, "supaterm-host-existing-{d}", .{std.c.getpid()});
