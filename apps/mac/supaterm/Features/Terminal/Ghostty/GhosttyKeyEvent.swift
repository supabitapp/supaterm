...
enum GhosttyKeyEvent {
  static func make(
    ...
    keyEvent.mods = mods(originalMods)
    keyEvent.consumed_mods = mods(translationMods.subtracting([.control, .command]))
  ...
  static func make(
    ...
    keyEvent.mods = mods(event.modifierFlags)
    keyEvent.consumed_mods = mods(event.modifierFlags.subtracting([.control, .command]))
    keyEvent.unshifted_codepoint = event.unshiftedCodepoint
    return keyEvent
  }
