...
enum TerminalSidebarLayout {
  ...
  static func spaceMonogram(for name: String, fallbackIndex: Int) -> String {
    let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
    return trimmed.first.map { String($0).uppercased() } ?? String(fallbackIndex + 1)
  }

  static func showsSpaceList(spacesCount: Int) -> Bool {
    spacesCount > 1
  }
}
