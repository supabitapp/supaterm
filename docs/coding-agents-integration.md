...
- Supaterm injects pane-local environment into terminal processes:
  ...
  - `SUPATERM_SURFACE_ID`
  - `SUPATERM_TAB_ID`
- Supaterm prepends its signed command directories to pane `PATH`. `Contents/MacOS` ships `sp` and the `ap` coding-agents session picker built from the `ThirdParty/coding-agents-session-picker` submodule. `Contents/MacOS/commands` ships `supaterm`, the Rust prompt TUI.
