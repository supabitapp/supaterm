...
- Managed app sockets share the `supaterm` directory in Darwin's per-user temporary root, so discovery does not depend on the hook host's `XDG_RUNTIME_DIR` or `TMPDIR`.
- The CLI polls each managed app socket within one routing budget. It removes a failed socket only when its PID is dead. Candidate order is one direct process match from a nonshared hook host; on a complete round, the existing owner for a same-ID `compact`; after the detection deadline, one exact full or Codex-rendered session ID token in the raw terminal title; for `startup`, one guarded fork; otherwise one workspace match. A custom display title does not replace the raw title used for matching.
