...
- A release stays included forever when its release date falls on or before the device license's update entitlement end date.
- The updater installs only releases included with the device license.
- An included release keeps paid mode after its update entitlement ends.

When an expired update entitlement is used with a newer release, Supaterm runs in free mode and offers two actions: renew the entitlement or download the latest release included with the license.

## Offline Use and Revocation

An activated, included release keeps paid mode without an internet connection. Failed license checks never remove paid access from an activated device.
...
A full renewal refund voids that renewal period. Supaterm recalculates the update end date from the remaining payments, and releases after the new end date are no longer included. If a later kept renewal spans the same dates, those dates remain included because the entitlement stores one end date rather than separate ranges.
