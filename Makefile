...
host-check:  # Format, lint, test, and build the host.
	@cd apps/supaterm-host && cargo fmt --check
	@cd apps/supaterm-host && cargo clippy --all-targets --all-features -- -D warnings
	@cd apps/supaterm-host && cargo test
	@$(MAKE) mac-build-host
